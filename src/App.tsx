...
import { History, FileText, X, CheckCircle, Sun, Moon, Zap, ShieldCheck, Activity, TrendingUp, Share2 } from 'lucide-react';
...
function App() {
  ...
  const aiSummary = `Our deep-scan AI audit indicates that by optimizing ${currentTool?.name}, your organization can recover $${result.savings * 12} annually. This optimization eliminates seat wastage and leverages Credex's tiered pricing model for maximum ROI.`;
    ...
    if (auditId) {
      const fetchPublicAudit = async () => {
        ...
        if (data) {
          ...
          setShowResults(true);
          toast.success("Viewing Shared Audit Report");
        }
        setLoading(false);
      };
      fetchPublicAudit();
  ...
  const handleGenerateAudit = async () => {
    ...
    try {
      const { data, error } = await supabase.from('audits').insert([{ email, tool, seats, savings: result.savings }]).select().single();
  ...
  const handleShare = () => {
    if (currentAuditId) {
      const shareUrl = `${window.location.origin}?id=${currentAuditId}`;
      navigator.clipboard.writeText(shareUrl);
      toast.success("Unique audit link copied to clipboard!");
    } else {
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-10">
        <nav className="flex justify-between items-center mb-8 bg-white/5 backdrop-blur-lg border border-white/10 p-3 rounded-full shadow-2xl">
          <div className="flex items-center gap-2 font-black text-xl tracking-tighter uppercase px-3">
            <div className="bg-blue-600 p-1.5 rounded-full shadow-lg shadow-blue-600/30"><Zap className="text-white fill-white" size={16} /></div>
            Credex<span className="text-blue-500 font-black">Audit</span>
          </div>
          <div className="flex items-center gap-1.5">
             <button onClick={() => setIsDark(!isDark)} className="p-2.5 rounded-full transition-all hover:bg-white/10">{isDark ? <Sun size={18} /> : <Moon size={18} />}</button>
             <button onClick={() => setShowHistory(true)} className="p-2.5 hover:bg-blue-500/10 rounded-full transition-all"><History size={18} /></button>
          </div>
        </nav>
        ...
        <div className="grid gap-10 lg:grid-cols-12 items-start">
          <div className="lg:col-span-7 space-y-10">
            ...
            <div className={`p-8 rounded-[3rem] border transition-all ${isDark ? 'bg-white/5 border-white/10 shadow-2xl' : 'bg-white border-slate-200 shadow-lg'}`}>
              <div className="space-y-6">
                <div><label className="text-[11px] font-black uppercase tracking-widest opacity-50">SaaS Hub</label>
                ...
                <div><label className="text-[11px] font-black uppercase tracking-widest opacity-50">Users: <span className="text-blue-500 text-lg ml-1 font-black">{seats}</span></label>
                  <input type="range" min="1" max="100" value={seats} onChange={(e) => setSeats(Number(e.target.value))} className="w-full h-3 bg-blue-500/10 rounded-full appearance-none cursor-pointer accent-blue-600 mt-4 transition-all" />
                </div>
                <div><label className="text-[11px] font-black uppercase tracking-widest opacity-50">Company Email</label>
                ...
                <button onClick={handleGenerateAudit} disabled={loading} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl hover:bg-blue-500 active:scale-95 transition-all shadow-xl shadow-blue-500/30 uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-4">
                  {loading ? <Activity className="animate-pulse" size={18} /> : <ShieldCheck size={18} />} {loading ? 'Performing AI Audit...' : 'Execute Official Audit'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 h-full">
            {showResults ? (
              <div className="animate-in fade-in zoom-in duration-700 h-full flex flex-col gap-6">
                <div className="bg-gradient-to-br from-blue-600 to-indigo-800 p-8 rounded-[3.5rem] text-white relative overflow-hidden shadow-2xl border border-white/20">
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-3">
                        ...
                        <button onClick={handleShare} className="p-2 bg-white/20 hover:bg-white/30 rounded-full transition-all" title="Share Unique Audit">
                            <Share2 size={16} />
                        </button>
                    </div>
                    <div className="text-6xl font-black tracking-tighter mb-8 leading-none flex items-start gap-1"><span className="text-2xl mt-1 opacity-50 font-bold">$</span>{result.savings * 12}</div>
                    <div className="p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 flex items-start gap-3">
                      ...
                      <div>
                         <p className="text-[10px] font-black text-blue-200 uppercase mb-1">AI-Generated Strategy</p>
                         <p className="text-xs font-bold leading-tight text-blue-50">{aiSummary}</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className={`p-8 rounded-[2.5rem] border backdrop-blur-xl transition-all ${isDark ? 'bg-white/5 border-white/10 shadow-lg' : 'bg-white border-slate-200'}`}>
                    <div className="space-y-6">
                      ...
                      <div className="pt-6 border-t border-dashed border-white/10 flex justify-between items-end">
                        ...
                        <div className="text-[10px] font-black px-3.5 py-1.5 bg-emerald-500/10 text-emerald-500 rounded-lg border border-emerald-500/20">30% DISCOUNT</div>
                      </div>
                    </div>
                </div>
              </div>
          ...
          <div className={`w-full max-w-sm h-full p-10 border-l animate-in slide-in-from-right duration-300 ${isDark ? 'bg-slate-900 border-white/10 shadow-2xl' : 'bg-white border-slate-200'}`}>
            ...
            <div className="space-y-4">
              {history.length > 0 ? history.map((item, index) => (
                ...
                <p className="text-center opacity-40 text-xs py-10">No audits found in sync.</p>
              )}
            </div>
          </div>
        </div>
